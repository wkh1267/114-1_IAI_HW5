...
from flowavenet.modules import Wavenet
...
class Flowavenet(nn.Module):
    ...
    def _pad_2d(self, x, max_len, b_pad=0):
        x = np.pad(x, [(b_pad, max_len - len(x) - b_pad), (0, 0)],
                mode="constant", constant_values=0)
        return x

    def inference(self, wav_file, mel_file, out_file):
        ...
        pad = (wav.shape[0] // hop_length + 1) * hop_length - wav.shape[0]
        pad_l = pad // 2
        pad_r = pad // 2 + pad % 2

        wav = np.pad(wav, (pad_l, pad_r), mode="constant", constant_values=constant_values)
        N = mel.shape[0]
        mel = mel[:-1,]
        wav = wav[:N * hop_length]
        ...
        q_0 = Normal(x_batch.new_zeros(x_batch.size()), x_batch.new_ones(x_batch.size()))
        z = q_0.sample() * 0.8
        if torch.cuda.is_available():
            torch.cuda.synchronize()

        with torch.no_grad():
            y_gen = self.reverse(z, c_batch).squeeze()
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        wav = y_gen.to(torch.device("cpu")).data.numpy()

        sf.write(out_file, wav, samplerate=22050)
